...
public class ExampleMod {
    ...
    public ExampleMod() {
        ...
        for (final IModInfo modInfo : modEntry.getMods()) {
            ...
            LOGGER.info("Loaded {} v{} from '{}'", modInfo.getModId(), modInfo.getVersion(),
                    modEntry.getFile().getFilePath());

            if (!verifyJarSignature(modEntry.getFile().getFilePath().toFile())) {

                LOGGER.error("Could not verify the mod's signature! modId={} version={} path={}", modInfo.getModId(),
                        modInfo.getVersion(), modEntry.getFile().getFilePath());
            }
        }
    }
